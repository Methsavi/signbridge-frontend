              ...
              <div className="relative flex flex-col items-center">
                <div className="relative cursor-pointer group" onClick={handleImageClick} title="Change Profile Picture">
                  {user.profile_picture ? (
                    <img src={user.profile_picture} alt="Profile" className="object-cover w-24 h-24 transition-opacity border-4 border-gray-800 rounded-full shadow-lg hover:opacity-90" />
                  ) : (
                ...
                {!isEditing ? (
                  <>
                    ...
                    <button onClick={() => setIsEditing(true)} className="flex items-center justify-center w-full gap-2 px-4 py-2 transition-colors bg-gray-700 rounded-lg hover:bg-gray-600">
                  ...
                  <div className="w-full mt-4 space-y-4">
                    <div>
                        ...
                        <input type="text" value={formData.username} onChange={(e) => setFormData({...formData, username: e.target.value})} className="w-full p-2 text-white bg-gray-900 border border-gray-600 rounded focus:border-primary focus:outline-none" />
                    </div>
                    <div>
                        <label className="text-xs font-semibold text-gray-500 uppercase">Email</label>
                        <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full p-2 text-white bg-gray-900 border border-gray-600 rounded focus:border-primary focus:outline-none" />
                    </div>
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setIsEditing(false)} className="flex items-center justify-center flex-1 gap-2 py-2 text-white bg-gray-700 rounded hover:bg-gray-600"><X className="w-4 h-4" /> Cancel</button>
                    </div>
                  </div>
                )}
              </div>
              <div className="pt-6 mt-8 border-t border-gray-700">
                <div className="flex items-center justify-between mb-2 text-sm text-gray-400"><span className="flex items-center gap-2"><Clock className="w-4 h-4" /> Joined</span><span>Jan 2026</span></div>
                <div className="flex items-center justify-between text-sm text-gray-400"><span className="flex items-center gap-2"><Calendar className="w-4 h-4" /> Translations</span><span className="font-bold text-primary">{history.length}</span></div>
              </div>
            </div>
          </motion.div>
              ...
              <div className="flex-grow p-4 space-y-3 overflow-y-auto custom-scrollbar">
                {loadingHistory ? (
                    ...
                    {history.length === 0 ? (
                        ...
                        history.map((item) => (
                        <motion.div key={item._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20, height: 0 }} className="relative p-4 transition-colors border border-gray-700 bg-gray-900/50 rounded-xl hover:border-primary/50 group">
                            <button onClick={() => handleDeleteItem(item._id)} className="absolute p-2 text-gray-600 transition-all rounded-lg opacity-0 top-4 right-4 hover:text-red-500 group-hover:opacity-100 hover:bg-gray-800"><Trash2 className="w-4 h-4" /></button>
                            <div className="flex items-start justify-between pr-8 mb-2"><span className="px-2 py-1 text-xs text-gray-500 bg-gray-800 rounded">{new Date(item.timestamp).toLocaleDateString()} • {item.target_language.toUpperCase()}</span></div>
                            <div className="flex items-center gap-4">
                            <div className="flex-1"><p className="mb-1 text-sm text-gray-400">Original Sign</p><p className="text-lg font-semibold text-white">{item.original_text}</p></div>
                            <ArrowRight className="w-5 h-5 text-gray-600 transition-colors group-hover:text-primary" />
                            <div className="flex-1 text-right"><p className="mb-1 text-sm text-gray-400">Translated</p><p className="text-lg font-semibold text-primary">{item.translated_text}</p></div>
                            </div>
                        </motion.div>
                        ))
                    )}
